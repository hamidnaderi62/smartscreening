...
def dashboard_detail_fa(request):
    ...
    idf_score_history = list(reversed([float(my_model.idf_score) for my_model in my_models]))
    premm_score_history = list(reversed([float(my_model.premm_score) for my_model in my_models]))
    # date_history = list(reversed([my_model.created.date().strftime("%Y-%m-%d") for my_model in my_models]))

    from persiantools.jdatetime import JalaliDate
    date_history = list(reversed([JalaliDate(my_model.created.date(), locale='fa').strftime("%Y-%m-%d") for my_model in my_models]))

    # stomach_cancer_score_history = [float(my_model.stomach_cancer_score) for my_model in my_models if my_model.stomach_cancer_score]
    print(idf_score_history)
